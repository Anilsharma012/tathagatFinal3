...
import { FaTrash, FaSearch, FaDownload, FaExternalLinkAlt, FaCheckSquare, FaSquare } from "react-icons/fa";
...
const ResponseSheetSubmissions = () => {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            const response = await axios.delete(`/api/v3/admin/submissions/${id}`);
            if (response.data.success) {
                fetchSubmissions();
                setSelectedIds(selectedIds.filter(i => i !== id));
            }
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            setDeleteLoading(true);
            const response = await axios.post(`/api/v3/admin/submissions/bulk-delete`, {
                ids: selectedIds
            });
            if (response.data.success) {
                fetchSubmissions();
    ...
    const exportToCSV = () => {
        ...
        const headers = ["Application No", "Candidate Name", "Roll No", "Subject", "Test Date", "Test Time", "Submitted At"];
        ...
        link.setAttribute("download", `response_sheet_submissions_${new Date().toISOString().split('T')[0]}.csv`);
            ...
            <div className="rss-container">
                <div className="rss-header">
                    <div className="rss-title-section">
                        ...
                        <span className="rss-count">{total} total submissions</span>
                    </div>
                ...
                <div className="rss-table-container">
                    {loading ? (
                        <div className="rss-loading">Loading submissions...</div>
                                    ...
                                    <tr key={submission._id}>
                                        <td className="rss-checkbox-col">
                                            <button 
                                                ...
                                                onClick={() => handleSelectOne(submission._id)}
                                            >
                                                {selectedIds.includes(submission._id) ? 
                                                    <FaCheckSquare className="checked" /> : <FaSquare />}
                                            </button>
                                        </td>
                                        <td>{submission.applicationNo || "-"}</td>
                                        <td className="rss-name-cell">{submission.candidateName || "-"}</td>
